%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Wenneker Resume/CV
% LaTeX Template
% Version 1.1 (19/6/2016)
%
% This template has been downloaded from:
% http://www.LaTeXTemplates.com
%
% Original author:
% Frits Wenneker (http://www.howtotex.com) with extensive modifications by 
% Vel (vel@LaTeXTemplates.com)
%
% License:
% CC BY-NC-SA 3.0 (http://creativecommons.org/licenses/by-nc-sa/3.0/
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%----------------------------------------------------------------------------------------
%	PACKAGES AND OTHER DOCUMENT CONFIGURATIONS
%----------------------------------------------------------------------------------------

\documentclass[a4paper,10pt]{memoir} % Font and paper size

\input{structure.tex} % Include the file specifying document layout and packages


\userinformation{ % Set the content that goes into the sidebar of each page
\begin{center}

%----------------------------------------------------------------------------------------
%	NAME AND CONTACT INFORMATION 
%----------------------------------------------------------------------------------------

\includegraphics[width=0.75\columnwidth]{portrait.png}\\[\baselineskip] % Your photo
\small % Smaller font size
\link{mailto:tcp510@york.ac.uk}{tcp510@york.ac.uk} \\ % Your email address
(+44) 77400 54268 \\ % Your phone number
York, UK \\
\Sep
\link{https://tompollak.me}{tompollak.me} \\ % Your URL
\link{https://github.com/tom-pollak}{github.com/tom-pollak} \\ % Your GitHub URL

%----------------------------------------------------------------------------------------
%	LANGUAGES, TECHNOLOGIES, AND INTERESTS
%----------------------------------------------------------------------------------------

\Sep \Sep
\textbf{Languages} \\
\SmallSep
Python, Java, Haskell, JavaScript, SQL, HTML, CSS \\
\Sep

\textbf{Technologies} \\
\SmallSep
Linux, Git, Vim, Emacs, LaTex, Django, Numpy, Pandas, LibGDX, Docker, Vue.js, Selenium \\
\Sep

\textbf{Interests} \\
\SmallSep
Mountain biking, Climbing, Poker, Chess, Running, Hockey \\
\end{center}
\vfill % Whitespace under this block to push it up under the photo
}

%----------------------------------------------------------------------------------------

\begin{document}

\userinformation % Print your information in the left column

\framebreak % End of the first column

%----------------------------------------------------------------------------------------
%	HEADING
%----------------------------------------------------------------------------------------

\cvheading{Tom Pollak} % Large heading - your name

\cvsubheading{CS Student @ University of York} % Subheading - your occupation/specialization

%----------------------------------------------------------------------------------------
%	ABOUT ME
%----------------------------------------------------------------------------------------

\aboutme{About Me}{
	A second year Computer Science student studying at the University of York
	seeking a summer internship in software development or other related fields.
	I enjoy learning practically, and have developed a number of personal
	projects to further my knowledge in areas that interest me. I am most
	proficient with Python but also have experience in Java and Haskell, with a
	working knowledge of JavaScript. I also have many years of experience with
	working with Linux systems and Git version control.
}

%----------------------------------------------------------------------------------------
%	EDUCATION
%----------------------------------------------------------------------------------------

\CVSection{Education}

%------------------------------------------------

\CVItem{University of York}{2020 - 2023}{https://york.ac.uk}{
	BEng Computer Science - Average 77\%, Expected First \\[2pt]
	\begin{tabular}{ p{0.2\textwidth} p{0.2\textwidth} p{0.3\textwidth} }
		\bluebullet Theory 1 (81\%) & \bluebullet Theory 2 (72\%) & \bluebullet Theory 3 (78\%) \\
		\bluebullet Data 1 (72\%) & \bluebullet Data 2 (72.5\%) & \bluebullet Intelligent Systems 1 (73\%)  \\
		\bluebullet Software 1 (95\%) & \bluebullet Software 2 (94\%) & \bluebullet Systems \& Devices 1 (78\%) \\
		\bluebullet \raggedright Human Computer \hspace*{8pt} Interaction 1 (66\%) & \bluebullet \raggedright Human Computer \hspace*{8pt} Interaction 2 (70\%) & \bluebullet Systems \& Devices 2 (71\%) \\
	\end{tabular}
}

%------------------------------------------------

\CVItem{Lady Manners School}{2020}{}{
	A-Level \& GCSE \\[2pt]
	\begin{tabular}{ p{0.3\textwidth} p{0.3\textwidth} }
		\bluebullet Further Maths (A) & \bluebullet Maths (A) \\
		\bluebullet Computer Science (A) & \bluebullet Physics (A) \\
		\bluebullet GCSE: 5 8s, 2 7s, 2 6s, 1 5. \\
	\end{tabular}
}


%----------------------------------------------------------------------------------------
%	EXPERIENCE
%----------------------------------------------------------------------------------------

\CVSection{Projects}

%------------------------------------------------

\CVItem{Each Way Matcher}{December 2020 - July 2021}{https://github.com/tom-pollak/each-way-matcher}{
	Automated horse racing program that discovers undervalued horses by the bookmaker in each-way betting.
	\begin{itemize}
		\item Uses an adapted Kelly Criterion strategy to calculate the optimal stake to profit of these undervalued horses.
		\item Runs headless on a Raspberry Pi as a scheduled cron job every day.
	\end{itemize}
}

%------------------------------------------------

\CVItem{Pirate Game}{January - February 2022}{https://github.com/tom-pollak/pirates}{
	Interactive pirate game using Java and LibGDX.
}

%------------------------------------------------

\CVItem{Poker Web Application}{April 2019 - July 2020}{https://github.com/tom-pollak/web-poker}{
	Free live poker web app using Python and Django.
	\begin{itemize}
		\item Users can create accounts and tables, play poker, chat with other players.
		\item Uses Django Channels to allow for real-time communication between users.
		\item Deployed using Docker and Heroku.
	\end{itemize}
}

%------------------------------------------------

\CVItem{SANS Institute, \textit{FOR500 Windows Forensic Analysis} }{August 2020}{https://www.sans.org/cyber-security-courses/windows-forensic-analysis}{
	\begin{itemize}
		\item Sponsored to take part through my success in the Cyber Discovery programme.
	\end{itemize}
}

%------------------------------------------------

\CVItem{Cyber Discovery}{September 2018 - July 2019}{https://joincyberdiscovery.com/}{
	Independently completed the Cyber Discovery programme, run by HM government.
	\begin{itemize}
		\item I was selected as one of the top 500 (of 28,000) students to attend the Cyber Discovery Elite event in London.
	\end{itemize}
}

%----------------------------------------------------------------------------------------
%	Work
%----------------------------------------------------------------------------------------

\CVSection{Work}

%------------------------------------------------

\CVItem{Glasshouse Bar, \textit{Bartender}}{September 2021 - Present}{}{
	Working as a bartender at the York University Student Union.
}

%------------------------------------------------

\CVItem{Seafood Bar \& Grill, \textit{Waiter}}{July - September 2021}{}{}

%------------------------------------------------

\CVItem{Derbyshire Police, \textit{Mock Police Witness Interviewee}}{March 2021}{}{
	Role acting as a witness for Derbyshire Police CID Interview Training.
	\begin{itemize}
		\item Involves being interviewed by police detectives in training, in order to help improve their interview technique on children.
		\item Requires proficient communication skills, and being able to communicate effectively whilst improvising details from a fictional crime scene.
	\end{itemize}
}

%------------------------------------------------

\CVItem{Fishpond, \textit{Bartender}}{June - September 2020}{}{}

%------------------------------------------------

\end{document}
